...
Receive, send, store, search, and synchronize email at scale. It separates SMTP/IMAP delivery concerns from metadata, attachments, caching, search indexing, and multi-data-center reliability.
...
This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Metadata database**. It explains **attachments** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 2. dns lookup

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Email knowledge 101**. It explains **dns lookup** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 3. elasticsearch

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Search**. It explains **elasticsearch** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 4. email receiving flkow

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Distributed mail servers**. It explains **email receiving flkow** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 5. email sending flow

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Distributed mail servers**. It explains **email sending flow** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 6. emails table

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Metadata database**. It explains **emails table** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 7. folders table

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Metadata database**. It explains **folders table** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 8. high level architecture

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **Distributed mail servers**. It explains **high level architecture** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 9. legend

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Metadata database**. It explains **legend** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 10. local dir storage

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Traditional mail servers**. It explains **local dir storage** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 11. lsm tree

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Search**. It explains **lsm tree** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 12. multi dc example

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Scalability and availability**. It explains **multi dc example** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 13. read unread emails

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Metadata database**. It explains **read unread emails** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 14. traditional mail server

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Traditional mail servers**. It explains **traditional mail server** as part of the distributed email service design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

## Source lesson text

_Source: `23. Distributed Email Service/README.md`. Embedded figures are replaced by the visual guide above; all written lesson content is retained below._
