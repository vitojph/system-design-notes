...
Move money between wallet shards while preserving balances and an audit trail. It compares distributed transactions, Try-Confirm-Cancel, Saga, CQRS, and high-performance replicated event sourcing.
...
This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Distributed transactions**. It explains **2pc protocol** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 2. cancel phase

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Distributed transaction using Try-Confirm/Cancel (TC/C)**. It explains **cancel phase** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 3. command queue

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Event sourcing**. It explains **command queue** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 4. confirm phase

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Distributed transaction using Try-Confirm/Cancel (TC/C)**. It explains **confirm phase** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 5. cqrs architecture

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **Event sourcing**. It explains **cqrs architecture** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 6. digital wallet

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Introduction**. It explains **digital wallet** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 7. distributed transactions relational dbs

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Distributed transactions**. It explains **distributed transactions relational dbs** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 8. dynamic event sourcing

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Event sourcing**. It explains **dynamic event sourcing** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 9. event sourcing

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Event sourcing**. It explains **event sourcing** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 10. historical states

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Event sourcing**. It explains **historical states** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 11. mmap optimization

This spatial or partitioning illustration. It divides a map, hash ring, or key space into cells or ranges and highlights how a lookup crosses boundaries or selects an owner in **High-performance event sourcing**. It explains **mmap optimization** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 12. out of order execution

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **TC/C Failure modes**. It explains **out of order execution** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 13. phase status tables

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **TC/C Failure modes**. It explains **phase status tables** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 14. polling approach

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Distributed event sourcing**. It explains **polling approach** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 15. push state machines

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Distributed event sourcing**. It explains **push state machines** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 16. raft replication

This replication and failure-handling diagram. It shows peer nodes and their links, then distinguishes normal replication or acknowledgement from a failed node, promotion, or recovery path in **Reliable high-performance event sourcing**. It explains **raft replication** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 17. reverse proxy

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Distributed event sourcing**. It explains **reverse proxy** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 18. rocks db approach

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **High-performance event sourcing**. It explains **rocks db approach** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 19. saga

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Distributed transaction using Saga**. It explains **saga** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 20. sharded raft groups

This replication and failure-handling diagram. It shows peer nodes and their links, then distinguishes normal replication or acknowledgement from a failed node, promotion, or recovery path in **Distributed event sourcing**. It explains **sharded raft groups** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 21. snapshot approach

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **High-performance event sourcing**. It explains **snapshot approach** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 22. try phase

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Distributed transaction using Try-Confirm/Cancel (TC/C)**. It explains **try phase** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 23. unbalanced state

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **TC/C Failure modes**. It explains **unbalanced state** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 24. wallet service state macghine

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **Event sourcing**. It explains **wallet service state macghine** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 25. wallet service

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **In-memory sharding solution**. It explains **wallet service** as part of the digital wallet design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

## Source lesson text

_Source: `27.  Digital Wallet/README.md`. Embedded figures are replaced by the visual guide above; all written lesson content is retained below._
