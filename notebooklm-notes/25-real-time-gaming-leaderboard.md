...
Accept score updates and return ranks or top players in real time. Compare relational, Redis sorted-set, DynamoDB, and sharded scatter-gather designs while preserving a fast hot path and durable history.
...
This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **High-level architecture**. It explains **alternative design** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 2. api gateway mapping

This spatial or partitioning illustration. It divides a map, hash ring, or key space into cells or ranges and highlights how a lookup crosses boundaries or selects an owner in **To use a cloud provider or not**. It explains **api gateway mapping** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 3. chess game leaderboard table 1

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Alternative solution: NoSQL**. It explains **chess game leaderboard table 1** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 4. chess game leaderboard table 2

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Alternative solution: NoSQL**. It explains **chess game leaderboard table 2** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 5. chess game leaderboard table 3

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Alternative solution: NoSQL**. It explains **chess game leaderboard table 3** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 6. dynamo db

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Alternative solution: NoSQL**. It explains **dynamo db** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 7. find leaderboard position

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Relational database solution**. It explains **find leaderboard position** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 8. hash partition

This spatial or partitioning illustration. It divides a map, hash ring, or key space into cells or ranges and highlights how a lookup crosses boundaries or selects an owner in **Scaling Redis**. It explains **hash partition** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 9. high level architecture

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **High-level architecture**. It explains **high level architecture** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 10. leaderboard position of user

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Redis solution**. It explains **leaderboard position of user** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 11. leaderboard table

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Relational database solution**. It explains **leaderboard table** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 12. leaderboard

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Introduction**. It explains **leaderboard** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 13. manage services ourselves

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **To use a cloud provider or not**. It explains **manage services ourselves** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 14. message queue based comm

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **High-level architecture**. It explains **message queue based comm** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 15. range partition

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Scaling Redis**. It explains **range partition** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 16. scatter gather 2

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Alternative solution: NoSQL**. It explains **scatter gather 2** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 17. skip list performance

This time-based comparison diagram. It uses a timeline, windows, axes, or side-by-side cases to expose the rate, capacity, latency, or scaling trade-off in **Redis solution**. It explains **skip list performance** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 18. skip list

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Redis solution**. It explains **skip list** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 19. sorted set

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Redis solution**. It explains **sorted set** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 20. top 10 players calculation

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Scaling Redis**. It explains **top 10 players calculation** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 21. user retrieve leaderboard

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **To use a cloud provider or not**. It explains **user retrieve leaderboard** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 22. user scoring point lambda

This spatial or partitioning illustration. It divides a map, hash ring, or key space into cells or ranges and highlights how a lookup crosses boundaries or selects an owner in **To use a cloud provider or not**. It explains **user scoring point lambda** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 23. user wins point

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Relational database solution**. It explains **user wins point** as part of the real-time gaming leaderboard design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

## Source lesson text

_Source: `25. Real-time Gaming Leaderboard/README.md`. Embedded figures are replaced by the visual guide above; all written lesson content is retained below._
