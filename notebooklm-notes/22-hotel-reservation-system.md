...
Search inventory and create reservations without double booking. The design centers on data modeling, idempotency, cache behavior, reservation state, locking or uniqueness constraints, and service-level consistency.
...
### 1. atomicity monolith

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Data consistency among services**. It explains **atomicity monolith** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 2. database constraint

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Option 3: Database constraints**. It explains **database constraint** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 3. database sharding

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Scalability**. It explains **database sharding** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 4. double booking multiple users

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Concurrency issues**. It explains **double booking multiple users** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 5. double booking single user

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Concurrency issues**. It explains **double booking single user** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 6. high level design

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **High-level Design**. It explains **high level design** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 7. idempotency

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Concurrency issues**. It explains **idempotency** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 8. inventory cache

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Scalability**. It explains **inventory cache** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 9. microservice non atomic operation

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Data consistency among services**. It explains **microservice non atomic operation** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 10. microservices vs monolith

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Data consistency among services**. It explains **microservices vs monolith** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 11. optimistic locking

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Option 2: Optimistic locking**. It explains **optimistic locking** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 12. pessimistic locking

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Option 1: Pessimistic locking**. It explains **pessimistic locking** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 13. qps estimation

This time-based comparison diagram. It uses a timeline, windows, axes, or side-by-side cases to expose the rate, capacity, latency, or scaling trade-off in **Back-of-the-envelope estimation**. It explains **qps estimation** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 14. schema design

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Data model**. It explains **schema design** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 15. status state machine

This state-machine or protocol timeline. It lays out the legal states or phases in order and uses alternate arrows to show the commit, retry, cancellation, or failure paths in **Data model**. It explains **status state machine** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 16. unique constraint violation

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Concurrency issues**. It explains **unique constraint violation** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 17. updated schema

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Improved data model**. It explains **updated schema** as part of the hotel reservation system design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

## Source lesson text

_Source: `22. Hotel Reservation System/README.md`. Embedded figures are replaced by the visual guide above; all written lesson content is retained below._
