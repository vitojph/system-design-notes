...
Accept and normalize orders, sequence them fairly, match them deterministically, publish market data, and persist an event stream. The design focuses on microsecond-scale performance, availability, order books, and reporting.
...
This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Performance**. It explains **application loop** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 2. candlestick

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Business Knowledge 101**. It explains **candlestick** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 3. client gateway

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Trading flow**. It explains **client gateway** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 4. client gateways

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Trading flow**. It explains **client gateways** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 5. design so far

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **Event sourcing**. It explains **design so far** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 6. determinism

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Determinism**. It explains **determinism** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 7. event sourcing

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Event sourcing**. It explains **event sourcing** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 8. high level design

This component architecture diagram. It arranges client-facing entry points, service boxes, queues or caches, and durable stores as separate layers; arrows identify the request or event paths between them in **High-level design**. It explains **high level design** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 9. l1 price

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Business Knowledge 101**. It explains **l1 price** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 10. l2 price

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Business Knowledge 101**. It explains **l2 price** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 11. l3 price

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Business Knowledge 101**. It explains **l3 price** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 12. leader election terms

This replication and failure-handling diagram. It shows peer nodes and their links, then distinguishes normal replication or acknowledgement from a failed node, promotion, or recovery path in **Fault tolerance**. It explains **leader election terms** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 13. leader election

This replication and failure-handling diagram. It shows peer nodes and their links, then distinguishes normal replication or acknowledgement from a failed node, promotion, or recovery path in **High availability**. It explains **leader election** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 14. market data publisher

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Market data publisher optimizations**. It explains **market data publisher** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 15. market data

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Market data flow**. It explains **market data** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 16. mmap bus

This spatial or partitioning illustration. It divides a map, hash ring, or key space into cells or ranges and highlights how a lookup crosses boundaries or selects an owner in **Performance**. It explains **mmap bus** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 17. order book execution

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Order book**. It explains **order book execution** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 18. order book impl

This data-model or worked-example diagram. It uses rows, columns, record fields, or sample values to show exactly what is stored or how a result is calculated in **Order book**. It explains **order book impl** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 19. product order execution data model

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Product, order, execution**. It explains **product order execution data model** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 20. replication across servers

This replication and failure-handling diagram. It shows peer nodes and their links, then distinguishes normal replication or acknowledgement from a failed node, promotion, or recovery path in **Fault tolerance**. It explains **replication across servers** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 21. reporting flow

This step-by-step flow diagram. Its arrows and, where present, numbered stages follow one request or event from its initiator through processing and storage to the resulting response or downstream action in **Reporting flow**. It explains **reporting flow** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 22. sequencer deep dive

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Event sourcing**. It explains **sequencer deep dive** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 23. sequencer

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Trading flow**. It explains **sequencer** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

### 24. world stock exchanges

This labeled conceptual diagram. It uses simple boxes, arrows, or a focused before-and-after view to isolate the mechanism rather than the whole system in **Introduction**. It explains **world stock exchanges** as part of the stock exchange design, so read the connections, ordering, partitions, or fields as the concrete representation of the surrounding discussion.

## Source lesson text

_Source: `28. Stock Exchange/README.md`. Embedded figures are replaced by the visual guide above; all written lesson content is retained below._
